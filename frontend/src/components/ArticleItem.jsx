...
const ArticleItem = ({ article }) => {
  const { id, title, subtitle, date, readTime, author, image, category } = article;
  ...
  return (
    <article className="flex flex-col rounded-[14px] border border-black/10 bg-white overflow-hidden hover:shadow-lg transition-shadow">
      ...
      <div className="relative">
        ...
        />
        {category && (
          <span className="absolute top-3 left-3 bg-[#159EEC] text-white text-xs font-medium font-inter px-3 py-1 rounded-full shadow-md">
            {category}
          </span>
        )}
      </div>
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <p className="text-[#4A5565] font-inter text-sm leading-5 flex-grow">
          {subtitle}
        </p>
