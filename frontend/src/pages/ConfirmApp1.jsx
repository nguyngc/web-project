...
function ConfirmApp1() {
  ...
  const handleConfirm = async () => {
    try {
      const appointmentData = { userId, doctorId, date, time, status };//title, doctor, 
      ...
      if (res.ok) {
        ...
        console.error("Failed to save appointment", res.status);
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-8">
            ...
            <div className="rounded-xl border border-black/10 bg-white">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <ul className="space-y-2">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="mt-1 inline-block w-2 h-2 rounded-full bg-[#159EEC]" />
                    <span className="text-[#101828] text-sm">Retinal examination</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-1 inline-block w-2 h-2 rounded-full bg-[#159EEC]" />
      ...
      {/* Actions */}
      <div className="flex flex-row flex-wrap justify-center gap-4 w-full max-w-[1040px] mx-auto p-6">
        <Link
          to="/profile"
